...
// Regulates low pH for increasing //       TO OPERATE THIS FEATURE, CLICK TOP RIGHT 
//    pH reactions... use acid     //       BUTTON (SERIAL MONITOR), INPUT VALUE & PRESS SEND.
...
//print out instructions for placing pH meter in solution
  ...
  if (statusValue == 1)                     //if '1' was pressed (pH Regulator)
   {
      ...
      Serial.println("You have entered: pH " + pHregulate);                  //display value inputted
...
void loop()   /*----( LOOP: RUNS CONSTANTLY )---------------------------------------------------------------------------------------*/
{
  ...
  if (statusValue == 1)                   //if user wants regulator
    {
      deltaPH = (pHvalue - pHregulate);   //difference between expected pH and actual pH
//  recheck:
...
//                    }
          ...
          else                                      //if difference in pH is between 0.3 and 1
            {
              for(int i=0; i < smallAdjust; i++)    //loop for smaller pH adjust time
                {
                  readPH();                         //check pH
                  Serial.println("pH Level = " + pHvalue);               //display value of pH
          ...
          for(int i=0; i < delayTime*4; i++)        //delay "delayTime" seconds to allow new solution to mix
                {
                  readPH();                         //check pH
                  Serial.println("pH Level = " + pHvalue);               //display value of pH
                  Serial.println("Delay for mixing (wait 100ms)");    //print "Delay for mixing"
                  delay(100);                       //small delay
                }
        }
    }
...
{
  Serial.println("Place pH meter in pH 4 solution...");
  ...
  Serial.println("Turn ADJ on pH chip");     //print out instructions for calibration of pH meter
  ...
  delay(50);                            //quick delay
  Serial.println("Place pH meter in pH 7 solution...");
  Serial.println("When ready input '1'");
  ...
  delay(50);                            //quick delay
  Serial.println("Turn ADJ on pH chip until reading is pH7");
  Serial.println("When done input '1'");
  ...
  delay(50);                            //quick delay
  Serial.println("Wash pH meter off with DI water.");
  Serial.println("When ready input '1'");
  ...
  delay(50);                            //quick delay
  Serial.println("Place pH meter in wanted solution.");
  Serial.println("When ready input '1'");
...
void CalibrateMeter2()   /*--(Subroutine, calibrates the pH meter using system of equations)----------------------------------------*/
{
  ...
  for (double i=100; i>0; i--)          //read values for 10 seconds
    {
      ...
      Serial.println("Time remaining: " + i/10 );               //calculates the time left for calibration
      Serial.println("pH Reading: " + pHvalue);               //current reading
    }
  ...
  delay(50);                            //quick delay
  Serial.println("Place pH meter in pH 10 solution...");
  Serial.println("When ready input '1'");
